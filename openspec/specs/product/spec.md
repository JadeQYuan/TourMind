...
|------------------|--------------|---------|----------------------------------|
| id               | 产品ID        | int     | 主键，自增                        |
| name             | 产品名称      | string  | 必填，唯一，1-64 字符              |
| days             | 天数          | int     | 必填，1-365，决定行程模板天数      |
| itinerary_template | 行程模板    | array   | 必填，数组，见下方说明             |
...
> itinerary_template 字段为数组，每条包含：seq（天序号，int）、details（行程内容，string，可空）等。

## Requirements


### Requirement: 产品列表分页查询
系统 SHALL 支持产品列表的分页展示，默认每页 20 条，并支持按产品名称等字段模糊搜索。

#### Scenario: 分页加载产品列表
- **WHEN** 用户进入产品管理页面
- **THEN** 系统自动加载第一页产品数据，显示分页控件，可切换页码加载对应数据
...
### Requirement: 新增/编辑产品
系统 SHALL 支持通过表单新增产品和编辑产品信息（如名称、天数、价格、备注等），名称不可与其他产品重复。新增产品时必填项校验，名称唯一。

#### Scenario: 新建产品表单校验
- **WHEN** 用户点击“新增产品”并填写表单后提交
- **THEN** 系统校验必填项，校验通过则保存新产品，失败则显示错误提示
...
### Requirement: 产品状态变更
系统 SHALL 支持管理员在产品列表中对产品进行禁用和启用操作，被禁用产品不可用于新订单，启用后可正常选择。

#### Scenario: 禁用产品
- **WHEN** 用户在产品列表点击“禁用”按钮
- **THEN** 系统将该产品状态设为禁用，后续新订单无法选择该产品

#### Scenario: 启用产品
- **WHEN** 用户在产品列表点击“启用”按钮
- **THEN** 系统将该产品状态设为启用，可正常选择
