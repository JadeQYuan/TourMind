...
| destination      | 目的地        | string  | 必填，自动联动订单产品            |
| travelers        | 出行人员      | string  | 必填，单行文本，逗号分隔           |
| days             | 天数          | int     | 必填，1-365，自动同步订单/产品天数  |
| details          | 行程明细      | array   | 必填，数组，每天一条，见下方说明    |
| status           | 状态          | string  | 必填，枚举：in_progress/completed/canceled |
| share_token      | 分享码        | string  | 自动生成，唯一，永久有效           |
| created_at       | 创建时间      | string  | 创建时间，ISO8601                  |
| updated_at       | 更新时间      | string  | 更新时间，ISO8601                  |

> details 字段为数组，每条包含：seq（天序号，int）、content（行程内容，string，可空）等。
...
仅 in_progress 状态下可见分享按钮。

#### Scenario: 状态判断分享按钮
- **WHEN** 行程状态为 in_progress
- **THEN** 显示分享按钮，否则隐藏

### Requirement: 行程状态变更
系统 SHALL 支持行程的完成、撤销、回退等状态变更，完成仅在所有明细日期已过时可用，撤销限时 24 小时内可用，已完成行程可回退为进行中。
