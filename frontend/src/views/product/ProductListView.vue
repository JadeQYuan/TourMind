...
const query = reactive({ keyword: '', status: 'active' as string | undefined })
...
async function openEdit(id: number) {
  try {
    ...
    form.value = {
      ...d,
      reference_price: d.reference_price ?? undefined,
...
const mobileFilter = reactive({ keyword: '', status: 'active' as string | undefined })

const activeFilterCount = computed(() => {
  let n = 0
  if (query.keyword) n++
  if (query.status !== 'active') n++
  return n
})
...
const columns = [
  ...
  { title: '参考价格(元)', dataIndex: 'reference_price', width: 110, align: 'right' as const,
  ...
  <div :style="{ padding: isMobile ? '12px 8px' : '24px' }">
    ...
    <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:12px;align-items:center;margin-bottom:16px">
      ...
      <a-form v-if="!isMobile" layout="inline">
        ...
        <a-form-item>
          <a-button @click="() => { query.keyword = ''; query.status = 'active'; pagination.current = 1; fetchList() }">重置</a-button>
        </a-form-item>
      </a-form>
      <!-- 移动端筛选按钮?-->
              ...
              <a-card size="small" style="border-radius:8px;border:1px solid #f0f0f0">
                <div style="display:flex;justify-content:space-between;align-items:flex-start">
                  <div>
                    ...
                    <div v-if="record.reference_price" style="font-size:12px;color:#0d9488;margin-top:2px">¥{{ Number(record.reference_price).toLocaleString() }}</div>
                  </div>
                  <a-tag :color="record.status === 'active' ? 'green' : 'default'" style="margin:0">{{ record.status === 'active' ? '上架' : '下架' }}</a-tag>
                </div>
                <div style="display:flex;justify-content:flex-end;gap:6px;margin-top:8px">
                  ...
                  <a-button size="small" :type="record.status === 'active' ? 'default' : 'primary'" @click="toggleStatus(record)">{{ record.status === 'active' ? '下架' : '上架' }}</a-button>
                </div>
              </a-card>
            </a-list-item>
          </template>
      ...
      <template #bodyCell="{ column, record, index }">
        ...
        <template v-else-if="column.key === 'action'">
          <a-space>
            <a-button size="small" @click="openViewDrawer(record.id)">查看</a-button>
            <a-button size="small" @click="openEdit(record.id)">编辑</a-button>
            <a-button size="small" :type="record.status === 'active' ? 'default' : 'primary'" @click="toggleStatus(record)">
      ...
      <a-spin :spinning="drawerLoading">
        <template v-if="drawerViewData">
          <a-descriptions :column="1" bordered size="small" style="margin-bottom:16px">
            ...
            <a-descriptions-item label="参考价格">
              {{ drawerViewData.reference_price != null ? `¥${Number(drawerViewData.reference_price).toLocaleString()}` : '-' }}
            </a-descriptions-item>
            <a-descriptions-item label="状态">
              <a-tag :color="drawerViewData.status === 'active' ? 'green' : 'default'">{{ drawerViewData.status === 'active' ? '上架' : '下架' }}</a-tag>
            </a-descriptions-item>
            ...
            <a-descriptions-item v-if="drawerViewData.notes" label="备注">
